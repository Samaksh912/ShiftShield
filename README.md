...
This repo is now set up for a two-service Render deployment via the root [render.yaml](/home/arnavbansal/Guidewire/render.yaml):

- `shiftshield-backend` as a public Node web service from [backend](/home/arnavbansal/Guidewire/backend)
- `shiftshield-ml` as a Python private service from [backend/ml-service](/home/arnavbansal/Guidewire/backend/ml-service)

Important deployment note:

- for a real deployed environment, set `SUPABASE_URL` and `SUPABASE_SERVICE_KEY`
- if you do not set them, the backend falls back to [backend/data/local-db.json](/home/arnavbansal/Guidewire/backend/data/local-db.json), which is ephemeral on Render and will reset on redeploy/restart
- this blueprint uses a private ML service, so the ML service is set to the `starter` plan instead of `free`

### Render steps

1. Push this repo with [render.yaml](/home/arnavbansal/Guidewire/render.yaml) to GitHub
2. In Render, choose `New +` -> `Blueprint`
3. Select the repo and branch you want to deploy
4. Render will create both services defined in the blueprint
5. During setup, provide values for:
   - `SUPABASE_URL`
   - `SUPABASE_SERVICE_KEY`
   - `FRONTEND_ORIGIN` set to your Vercel app URL, for example `https://your-app.vercel.app`
6. Complete the blueprint deploy

### What the blueprint already handles

- backend binds to `0.0.0.0` on Render
- backend gets a generated `JWT_SECRET`
- backend discovers the ML service through Render service discovery
- ML service starts with `uvicorn main:app --host 0.0.0.0 --port $PORT`
- ML service is pinned to internal port `8001`
- backend health check is `/health`
- ML runs as a private service on the `starter` plan

### Post-deploy env expectations
...
- `ML_SERVICE_HOSTPORT` injected from the Render ML service
- `SUPABASE_URL` set manually
- `SUPABASE_SERVICE_KEY` set manually
- `FRONTEND_ORIGIN` set manually for CORS
...
Once deployed, point the frontend to the Render backend URL:

- `https://<your-backend-service>.onrender.com`

If your frontend is hosted on Vercel, set:

- `FRONTEND_ORIGIN=https://<your-frontend>.vercel.app`

You can also allow multiple origins by setting:
